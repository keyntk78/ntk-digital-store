...
const ProductCategoryAdmin = () => {
  const theads = [
    ...
    { id: 3, nameKey: 'slug', title: 'Slug', isSort: true },
    { id: 4, nameKey: 'position', title: 'Vị trí', isSort: true }
  ]
  ...
  const onClickDelete = (id, name) => {
    const deleted = async () => {
      showAlert({
        subject: `Xóa loại sản phẩm [${name}]`,
        message: `Bạn có muốn xóa [${name}]?`,
        status: 'info',
        type: 'confirmation',
        onSuccess: async () => {
          const response = await makeRequest(apiDeleteProductCategory, id)
          if (response.success) {
            showToast({
              type: 'success',
              message: `Xóa loại sản phẩm [${name}] thành công`
            })
            setReloadData(true)
          } else {
            showAlert({
              subject: `Xóa loại sản phẩm ${name} thất bại`,
  ...
  return (
    <>
      ...
      <Modal showModal={showModal} size={'md'}>
        <Modal.Header
          setShowModal={setShowModal}
          color={selected ? 'secondary' : 'primary'}
        >
          {selected ? 'Cập nhật loại sản phẩm' : 'Thêm loại sản phẩm'}
        </Modal.Header>
        <Modal.Body>
          <FormProductCategory
            onSubmit={onSubmit}
